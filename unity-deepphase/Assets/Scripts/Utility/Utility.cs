...
public static class Utility {
	...
	public static double[] Polynomial(double[] x, double[] y, int grade) {
		double[] weights = MathNet.Numerics.Fit.Polynomial(x, y, grade);
		double[] values = new double[y.Length];
		for(int i=0; i<values.Length; i++) {
			values[i] = (float)MathNet.Numerics.Polynomial.Evaluate(x[i], weights);
		}
		return values;
	}
