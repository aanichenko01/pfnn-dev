	...
	public class RootModule : Module
	{
		...
		private Vector3 RootPosition(float timestamp, bool mirrored, TOPOLOGY topology)
		{
			...
			if (timestamp < start || timestamp > end)
			{
				...
				Vector3 interpolatedPostion = 2f * RootPosition(Asset.GetFrame(boundary)) - RootPosition(Asset.GetFrame(clamped));
				return Utility.ProjectGround(interpolatedPostion, Ground);
			}
			else
			{
			...
			Vector3 RootPosition(Frame frame)
			{
				...
				// 	position.y = 0f;
				// } else {
				// Always project onto the ground as most reflective of how controllers work
