...
namespace AI4Animation
{
	...
	public class MotionEditor : MonoBehaviour
	{
		...
		public bool ExportableOnly = false;
		...
		private string FootContactReferenceBone = "AnzB:LeftFootIndex4";
		...
		public class AssetSession
		{
			...
			public void Unload()
			{
				...
				// If jump target has been assigned
				if (asset.JumpTarget != null)
				{
					// Destroy Jump Target if Toggle is not on
					asset.DestroyJumpTarget();
				}
			...
			public void LoadFrame()
			{
				...
				for (int i = 0; i < GetActor().Bones.Length; i++)
				{
					...
					{
						GetActor().Bones[i].SetTransformation(
							Asset.GetFrame(Editor.GetTimestamp()).GetBoneTransformation(GetBoneMapping()[i], Editor.Mirror)
						);
						GetActor().Bones[i].SetVelocity(
							Asset.GetFrame(Editor.GetTimestamp()).GetBoneVelocity(GetBoneMapping()[i], Editor.Mirror)
						);
						// GetActor().Bones[i].SetAcceleration(
						// 	Asset.GetFrame(Editor.GetTimestamp()).GetBoneAcceleration(GetBoneMapping()[i], Editor.Mirror)
						// );
					}
				}
		...
		void Update()
		{
			...
			{
				LoadFrame(Mathf.Repeat(Timestamp + Time.deltaTime, GetSession().Asset.Frames.Last().Timestamp));
			}
		}

		public void LoadSession(string guid)
		{
			if (Session != null)
			{
				CloseSession();
		...
		public void Import()
		{
			...
			if (ExportableOnly)
			{
				List<string> exportable = new List<string>();
				for (int i = 0; i < Assets.Count; i++)
				{
					if (MotionAsset.Retrieve(Assets[i]).Export)
					{
						exportable.Add(Assets[i]);
					}
				}
				Assets = exportable;
				Resources.UnloadUnusedAssets();
		...
		public void LoadFrame(float timestamp)
		{
			if (GetSession() != null)
			{
				...
				if (ShowInRootSpace)
				{
					...
					GetSession().GetActor().transform.position = root.GetPosition().PositionFromTo(root, Matrix4x4.identity);
					GetSession().GetActor().transform.rotation = root.GetRotation().RotationFromTo(root, Matrix4x4.identity);
				}
			}
		}
		...
		public class MotionEditor_Editor : Editor
		{
			...
			public void Refresh()
			{
				...
				if (Target.Session != null && Target.Session.Asset == null)
				{
					Debug.Log("DETECTED");
					Target.Import();
				}
				...
				using (new EditorGUILayout.VerticalScope("Box"))
				{
					...
					using (new EditorGUILayout.VerticalScope("Box"))
					{
						...
						using (new EditorGUILayout.VerticalScope("Box"))
						{
							...
							if (Utility.GUIButton("Exportable Only", Target.ExportableOnly ? UltiDraw.Cyan : UltiDraw.DarkGrey, Target.ExportableOnly ? UltiDraw.Black : UltiDraw.White))
							{
								Target.ExportableOnly = !Target.ExportableOnly;
							}
							EditorGUILayout.EndHorizontal();
							for (int i = 0; i < Target.Directories.Count; i++)
							{
								AssetDirectory directory = Target.Directories[i];
								EditorGUILayout.BeginHorizontal();
							...
							if (Target.Settings)
							{
								...
								using (new EditorGUILayout.VerticalScope("Box"))
								{
									...
									using (new EditorGUILayout.VerticalScope("Box"))
									{
										...
										if (Utility.GUIButton("Print Indices", UltiDraw.DarkGrey, UltiDraw.White))
										{
											foreach (TimeSeries.Sample sample in Target.GetTimeSeries().Samples)
											{
												Debug.Log("Sample " + sample.Index + ": " + Target.GetSession().Asset.GetFrame(Target.GetTimestamp() + sample.Timestamp).Index);
											}
										}
									}
									Target.AutoPlay = EditorGUILayout.Toggle("Auto Play", Target.AutoPlay);
									...
									Target.FootContactReferenceBone = EditorGUILayout.TextField("Foot Contact Reference", Target.FootContactReferenceBone);
