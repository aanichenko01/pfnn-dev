...
namespace PFNN_DEV {
	public class BioAnimation_F42_v5_0 : MonoBehaviour {
			...
			*/
			...
			if(NN.Parameters != null) {
				...
				for(int i=0; i<PointSamples; i++) {
					...
					NN.SetInput(start + i*TrajectoryDimIn + 0, UnitScale * pos.x);
					NN.SetInput(start + i*TrajectoryDimIn + 1, UnitScale * pos.y);
					NN.SetInput(start + i*TrajectoryDimIn + 2, UnitScale * pos.z);
					NN.SetInput(start + i*TrajectoryDimIn + 3, dir.x);
					NN.SetInput(start + i*TrajectoryDimIn + 4, dir.z);
					NN.SetInput(start + i*TrajectoryDimIn + 5, Trajectory.Points[i*PointDensity].GetSlope());
				...
				for(int i=0; i<Actor.Bones.Length; i++) {
					...
					NN.SetInput(start + i*JointDimIn + 0, UnitScale * pos.x);
					NN.SetInput(start + i*JointDimIn + 1, UnitScale * pos.y);
					NN.SetInput(start + i*JointDimIn + 2, UnitScale * pos.z);
					NN.SetInput(start + i*JointDimIn + 3, UnitScale * vel.x);
					NN.SetInput(start + i*JointDimIn + 4, UnitScale * vel.y);
				...
				for(int i=RootPointIndex+1; i<Trajectory.Points.Length; i++) {
					...
					// Debug.Log($"Index:{i}, PrevSampleIndex X: {start + (prevSampleIndex-6)*TrajectoryDimOut + 0}, NextSampleIndex X: {start + (nextSampleIndex-6)*TrajectoryDimOut + 0}");
					float posX = (1-m) * NN.GetOutput(start + (prevSampleIndex-6)*TrajectoryDimOut + 0) + m * NN.GetOutput(start + (nextSampleIndex-6)*TrajectoryDimOut + 0);
					float posZ = (1-m) * NN.GetOutput(start + (prevSampleIndex-6)*TrajectoryDimOut + 1) + m * NN.GetOutput(start + (nextSampleIndex-6)*TrajectoryDimOut + 1);
					float dirX = (1-m) * NN.GetOutput(start + (prevSampleIndex-6)*TrajectoryDimOut + 2) + m * NN.GetOutput(start + (nextSampleIndex-6)*TrajectoryDimOut + 2);
					float dirZ = (1-m) * NN.GetOutput(start + (prevSampleIndex-6)*TrajectoryDimOut + 3) + m * NN.GetOutput(start + (nextSampleIndex-6)*TrajectoryDimOut + 3);
	...
	#if UNITY_EDITOR
	[CustomEditor(typeof(BioAnimation_F42_v5_0))]
	public class BioAnimation_F42_v5_0_Editor : Editor {

			public BioAnimation_F42_v5_0 Target;

			void Awake() {
				Target = (BioAnimation_F42_v5_0)target;
			}
